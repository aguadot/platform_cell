...
class Example(QMainWindow):
    ...
    def initUI(self):
        ...
        self.autofocus.move(190,420)
        self.autofocus.clicked.connect(self.autofocu)
        self.autofocus.setEnabled(False)

        self.liberar = QPushButton("Liberar-enfoque", self)
        self.liberar.move(190, 455)
        self.liberar.clicked.connect(self.liberar_enfoque)
        self.liberar.setEnabled(False)
    ...
    def liberar_enfoque(self):
        sender = self.sender()
        num_pasos='0'
        try:
            if sender.text()=='Liberar-enfoque':
                self.liberar.setText('Conectar-enfoque')
                enfocar='10'
                self.enf_5.setEnabled(False)
                self.enf_1.setEnabled(False)
                self.enf_inv_1.setEnabled(False)
                ...
                self.con_auto.setEnabled(False)
                self.liberar.setEnabled(True)
                mov_enfoque = '{0},{1}'.format(enfocar,
                                               num_pasos)  # enviamos la trama con la secuencia de enfocar siempre es un 8 y el nº de pasos a mover
                # print(bytes(trama_completa, 'utf-8'))
                ser.write(bytes(mov_enfoque, 'utf-8'))

            else:
                self.liberar.setText('Liberar-enfoque')
                enfocar='11'
